...
until confd -onetime -node $ETCD_HOST:$ETCD_PORT -config-file /etc/confd/conf.d/haproxy.toml; do
  echo "[mesos-kubernetes-proxy] Waiting for confd to refresh haproxy.cfg"
  sleep 5
done

# Run confd in the background to watch the upstream servers
confd -interval 10 -node $ETCD_HOST:$ETCD_PORT -config-file /etc/confd/conf.d/haproxy.toml &
